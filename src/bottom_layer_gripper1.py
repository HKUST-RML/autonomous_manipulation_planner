...
def Feasible_Grasp(current_vertex_position, F1_position, F2_position, profile_environment, accuracy = 0.002):
    if F1_position != None and F2_position != None:
        ...
        if Point(F1_position).distance(Point(F2_position))>0.82:
            return False
    if F1_position != None and LineString(profile_environment).distance(Point(F1_position))<0.1:
        return False
    if F2_position != None and LineString(profile_environment).distance(Point(F2_position))<0.1:
        return False
    if F1_position != None and Polygon(current_vertex_position).buffer(-accuracy).intersects(Point(F1_position))==True:
        return False
    if F2_position != None and Polygon(current_vertex_position).buffer(-accuracy).intersects(Point(F2_position))==True:
        return False
...
def system_config_list_tilting_slide(current_vertex_position, current_F1, current_F2, target_contact_state, total_angle, profile_environment):
    ...
    contact_edge_point['E1'], contact_edge_point['E2']=Tilting_slide_contact_edge_point(current_vertex_position, target_contact_state, profile_environment)
    if total_angle>0:
        rotate_step = 0.5
    else:
        rotate_step = -0.5
...
def SuitableGrasp_tilting_slide_instantaneous(current_vertex_position, current_F1, current_F2, ini_vertex_position, ini_CoM_position, target_contact_state, rotate_angle_direction, profile_environment):
    contact_index, contact_list, contact_normal_list, contact_local_tangent_list = contact_points_and_normal(current_vertex_position, current_F1, current_F2, profile_environment)
    contact_index_E = []
    contact_list_E = []
    contact_normal_list_E = []
...
def AdjustGraspAction_feasible_list(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, current_F1_position, current_F2_position):
    ...
    for action in ['PivotF1CW','PivotF1CCW','PivotF2CW','PivotF2CCW','SlideF1+', 'SlideF1-','SlideF2+', 'SlideF2-', 'Slide+PivotF1CW', 'Slide+PivotF1CCW', 'Slide-PivotF1CW', 'Slide-PivotF1CCW','Slide+PivotF2CW', 'Slide+PivotF2CCW', 'Slide-PivotF2CW', 'Slide-PivotF2CCW']:
...
def whether_AdjustGraspAction_feasible(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, action, current_F1_position, current_F2_position):
    ...
    if contact_finger_number==0:
        if action == 'PivotF1CW' or action == 'PivotF1CCW' or action == 'PivotF2CW' or action == 'PivotF2CCW':
            return True
        ...
        if action == 'PivotF1CW' or action == 'PivotF1CCW':
            ...
            if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode1)==True:
                return True
        if action == 'PivotF1CW' or action == 'PivotF1CCW':
            contact_mode1 = dict({'F1':'S+', 'F2': 'B'}, **dict.fromkeys(contact_index_E, 'R'))
            if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode1)==True:
                return True
        if action == 'PivotF1CW' or action == 'PivotF1CCW':
            contact_mode1 = dict({'F1':'S-', 'F2': 'B'}, **dict.fromkeys(contact_index_E, 'R'))
        ...
        if action == 'PivotF2CW' or action == 'PivotF2CCW':
            ...
            if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode2)==True:
                return True
        if action == 'PivotF2CW' or action == 'PivotF2CCW':
            contact_mode2 = dict({'F1':'B', 'F2': 'S+'}, **dict.fromkeys(contact_index_E, 'R'))
            if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode2)==True:
                return True
        if action == 'PivotF2CW' or action == 'PivotF2CCW':
            contact_mode2 = dict({'F1':'B', 'F2': 'S-'}, **dict.fromkeys(contact_index_E, 'R'))
            if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode2)==True:
                return True
        if action == 'SlideF1+' or action == 'SlideF1-' or action == 'SlideF2+' or action == 'SlideF2-':
            ...
            if abs(F1_contact_normal[0] + F2_contact_normal[0])<0.0001 and abs(F1_contact_normal[1] + F2_contact_normal[1])<0.0001:
                ...
                if action == 'SlideF1+':
                    ...
                    if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode3)==True:
                        return True
        if action == 'Slide+PivotF1CW' or action == 'Slide+PivotF1CCW' or action == 'Slide-PivotF1CW' or action == 'Slide-PivotF1CCW' or action == 'Slide+PivotF2CW' or action == 'Slide+PivotF2CCW' or action == 'Slide-PivotF2CW' or action == 'Slide-PivotF2CCW':
            F1_contact_normal = contact_normal_list[contact_index.index('F1')]
            F2_contact_normal = contact_normal_list[contact_index.index('F2')]
    ...
    if action == 'PivotF1CCW':
        ...
        for angle in range(1,360):
            ...
            if Point(target_F2).distance(LineString(profile_environment))<0.1:
                ...
                break
            if angle<5 and Polygon(current_vertex_position).buffer(-0.000001).intersects(Point(target_F2))==True:
                action_feasible=False
                break
        ...
        for angle in range(1,360):
            ...
            if Point(target_F2).distance(LineString(profile_environment))<0.1:
                action_feasible=False
                break
            if angle<5 and Point(target_F2).intersects(Polygon(current_vertex_position).buffer(-0.000001))==True:
                action_feasible=False
                break
            elif angle>4 and Point(target_F2).distance(Polygon(current_vertex_position))<0.005:
                cost = LineString([current_F1, current_F2]).length*angle*pi/180
        ...
        for angle in range(1,360):
            ...
            if Point(target_F1).distance(LineString(profile_environment))<0.1:
                action_feasible=False
                break
            if angle<5 and Point(target_F1).intersects(Polygon(current_vertex_position).buffer(-0.000001))==True:
                action_feasible=False
                break
        ...
        if action_feasible==True:
            target_position_set.append([target_F1, target_F2, cost]) 
    elif action == 'SlideF1+' or action == 'SlideF1-':
        ...
        if action == 'SlideF1+' and np.cross(F1_to_center, slide_direction)>0:
            slide_direction = [-slide_direction[0], -slide_direction[1]]
        elif action == 'SlideF1-' and np.cross(F1_to_center, slide_direction)<0:
            slide_direction = [-slide_direction[0], -slide_direction[1]]
        ...
        for sliding_length in np.arange(0.05, 0.8, 0.05):
            ...
            if Feasible_Grasp(current_vertex_position, target_F1, target_F2, profile_environment)==False:
                break
            target_position_set.append([target_F1, target_F2, cost])
    elif action == 'SlideF2+' or action == 'SlideF2-':
        min_distance = float('inf')
        for i in range(len(current_vertex_position)):
        ...
        if action == 'SlideF2+' and np.cross(F2_to_center, slide_direction)>0:
            slide_direction = [-slide_direction[0], -slide_direction[1]]
        elif action == 'SlideF2-' and np.cross(F2_to_center, slide_direction)<0:
            slide_direction = [-slide_direction[0], -slide_direction[1]]
        temp_contact_index, temp_contact_list, temp_contact_normal_list, temp_contact_local_tangent_list = contact_points_and_normal(current_vertex_position, current_F1, current_F2, profile_environment)
        temp0_contact_index = temp_contact_index[:]
        temp0_contact_list = temp_contact_list[:]
